...
var searchTerm = ""
function ebaySearch() {
    var queryURL = "http://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsByKeywords&sortOrder=PricePlusShippingLowest&buyerPostalCode=92128&SERVICE-VERSION=1.13.0&SECURITY-APPNAME=RyanChes-EbaySear-PRD-d13d69895-95fa1322&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&keywords=" + searchTerm   
    ...
    $.ajax({
      ...
      dataType: 'JSONP',
        ...
        for (i=0; i<response.findItemsByKeywordsResponse[0].searchResult[0].item.length; i++) {
            ...
            newDiv.attr("data-content", "<a> Price:  $"+ response.findItemsByKeywordsResponse[0].searchResult[0].item[i].sellingStatus.currentPrice + "</a> <br> <img height='140px' src="+ response.findItemsByKeywordsResponse[0].searchResult[0].item[i].galleryURL + ">")
